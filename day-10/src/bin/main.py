with open("src/bin/input.txt", "r") as f:
...
while True:
    ...
    unvisited_neighbors = [neighbor for direction in possible_directions if (neighbor := move(current_coords, direction)) not in visited]
...
for x in range(width):
    for y in range(height):
        ...
        if char == "S":
            ...
            start_pipe_type = [char for char, directions in direction_mappings.items() if set(directions) == set(start_options)][0]
